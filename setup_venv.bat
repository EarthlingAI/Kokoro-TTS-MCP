...
if errorlevel 1 (
	...
	echo   claude mcp add-json speak-tts "{\"type\":\"stdio\",\"command\":\"python\",\"args\":[\"%cd%\\server.py\"]}" --scope user
) else (
	echo Registering with Claude Code...
	claude mcp remove speak-tts --scope user >nul 2>&1
	claude mcp add-json speak-tts "{\"type\":\"stdio\",\"command\":\"python\",\"args\":[\"%cd%\\server.py\"]}" --scope user
	echo Registered.
)
echo.
